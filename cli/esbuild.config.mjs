 ...
 *
 * Bundles all workspace packages (@business-factory/*) into a single file.
...
// to npm and resolved at runtime (e.g. @business-factory/server uses dynamic import).
const externalWorkspacePackages = new Set([
  "@business-factory/server",
]);
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@business-factory/")) {
      externals.add(name);
    }
  }
