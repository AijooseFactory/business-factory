...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@business-factory/server/src/index.ts")) {
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @business-factory/server package
  try {
    const mod = await import("@business-factory/server");
    return await startServerFromModule(mod, "@business-factory/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@business-factory/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Paperclip server entrypoint.\n` +
          `Tried: ${devEntry}, @business-factory/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
