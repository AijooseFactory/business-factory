...
import { execute, testEnvironment } from "@business-factory/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@business-factory/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  ...
  it("drops unsupported payloadTemplate keys before invoking the gateway", async () => {
    const gateway = await createMockGatewayServer();
    const logs: string[] = [];
    ...
    try {
        ...
        buildContext(
          {
            ...
            payloadTemplate: {
              agentId: "oliver-wendell",
              extraSystemPrompt: "Stay structured and concise.",
              metadata: { team: "platform" },
              paperclip: { taskId: "should-not-be-forwarded" },
              message: "Follow the TPM operating model.",
            },
            waitTimeoutMs: 2000,
          },
          {
      ...
      expect(payload).toEqual(
        expect.objectContaining({
          ...
          sessionKey: "agent:oliver-wendell:paperclip:issue:issue-123",
          timeout: 2000,
        }),
      );
      expect(payload).not.toHaveProperty("metadata");
      expect(payload).not.toHaveProperty("paperclip");
      expect(logs.some((entry) => entry.includes("ignored unsupported payloadTemplate keys: metadata"))).toBe(true);
    } finally {
      await gateway.close();
    }
  });

  it("scopes legacy session keys to the requested agent when agentId is provided in config", async () => {
    const gateway = await createMockGatewayServer();
