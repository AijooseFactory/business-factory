...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Suggested Business Factory base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("businessFactoryApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.businessFactoryApiUrl");
    expect(text).toContain("~/.openclaw/workspace/business-factory-claimed-api-key.json");
    expect(text).toContain("BUSINESS_FACTORY_API_KEY");
  ...
  it("includes loopback diagnostics for authenticated/private onboarding", () => {
    ...
    expect(text).toContain("Connectivity Check");
