...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.BUSINESS_FACTORY_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  console.log('getWorktreeUiBranding env:', env);
  const name = "business-factory-pr-432";
  const color = normalizeHexColor(env.BUSINESS_FACTORY_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="business-factory-worktree-enabled" content="true" />',
    `<meta name="business-factory-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="business-factory-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="business-factory-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
