...
import { projects, projectGoals, goals, projectWorkspaces, workspaceRuntimeServices } from "@business-factory/db";
import {
  PROJECT_COLORS,
  deriveProjectUrlKey,
  isUuidLike,
...
function normalizeWorkspaceCwd(value: unknown): string | null {
  ...
  if (!cwd) return null;
  return cwd === REPO_ONLY_CWD_SENTINEL ? null : normalizeSharedProjectPath(cwd);
}
