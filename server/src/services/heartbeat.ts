...
import { extractPaperclipIssueUpdate } from "./agent-zero-task-result.js";
import { issueService } from "./issues.js";
import { isOnboardingBootstrapIssueForAgent } from "./onboarding-bootstrap.js";
import {
...
type ProjectWorkspaceSelection = Pick<
  ResolvedWorkspaceForRun,
  "cwd" | "workspaceId" | "repoUrl" | "repoRef" | "workspaceHints" | "warnings"
>;

type ProjectWorkspaceSelectionInput = {
  adapterType: string;
...
export async function resolveProjectWorkspaceSelection(
  input: ProjectWorkspaceSelectionInput,
): Promise<ProjectWorkspaceSelection | null> {
  if (input.projectWorkspaceRows.length === 0) return null;

  const workspaceHints = input.projectWorkspaceRows.map((workspace) => ({
    workspaceId: workspace.id,
    cwd: normalizeSharedProjectPath(readNonEmptyString(workspace.cwd)),
    repoUrl: readNonEmptyString(workspace.repoUrl),
    repoRef: readNonEmptyString(workspace.repoRef),
  }));
  const primaryWorkspace = input.projectWorkspaceRows[0] ?? null;
  const adapter = getServerAdapter(input.adapterType);
  const requiresServerLocalWorkspace = adapter.usesServerLocalWorkspace !== false;
  ...
  for (const workspace of input.projectWorkspaceRows) {
    const projectCwd = normalizeSharedProjectPath(readNonEmptyString(workspace.cwd));
    if (!projectCwd || projectCwd === REPO_ONLY_CWD_SENTINEL) {
      continue;
    }
    hasConfiguredProjectCwd = true;
    if (!requiresServerLocalWorkspace || (await directoryExists(projectCwd))) {
      return {
        cwd: projectCwd,
        workspaceId: workspace.id,
        repoUrl: readNonEmptyString(workspace.repoUrl),
  ...
  return {
    ...
    repoUrl: readNonEmptyString(primaryWorkspace?.repoUrl),
    repoRef: readNonEmptyString(primaryWorkspace?.repoRef),
    workspaceHints,
    warnings,
  };
}
...
export function shouldResetTaskSessionForWake(
  ...
  if (wakeReason === "issue_updated") return true;
...
function describeSessionResetReason(
  ...
  if (wakeReason === "issue_updated") return "wake reason is issue_updated";
...
export function mergeIssueContext(
  context: Record<string, unknown>,
  issue: IssueContextRef | null,
) {
  if (!issue) return context;

  const existingIssue = parseObject(context.issue);
  context.issue = {
    ...existingIssue,
    id: issue.id,
    ...(readNonEmptyString(issue.identifier) ? { identifier: issue.identifier } : {}),
    ...(readNonEmptyString(issue.title) ? { title: issue.title } : {}),
    ...(readNonEmptyString(issue.description) ? { description: issue.description } : {}),
  };
  if (!readNonEmptyString(context.issueId)) {
...
export function heartbeatService(db: Db) {
  ...
  async function resolveWorkspaceForRun(
    ...
    if (projectWorkspaceRows.length > 0) {
      ...
      const selectedWorkspace = await resolveProjectWorkspaceSelection({
        adapterType: agent.adapterType,
        projectWorkspaceRows: projectWorkspaceRows.map((workspace) => ({
          id: workspace.id,
          cwd: workspace.cwd,
      ...
      if (selectedWorkspace) {
        return {
          ...
          workspaceHints: selectedWorkspace.workspaceHints,
          warnings: selectedWorkspace.warnings,
        };
      }
  ...
  async function executeRun(runId: string) {
    ...
    const projectExecutionWorkspacePolicy = parseProjectExecutionWorkspacePolicy(
      projectRecord?.executionWorkspacePolicy,
    );
    ...
    const issueRef = issueId
      ? await db
          ...
          .then((rows) => rows[0] ?? null)
      : null;
    mergeIssueContext(context, issueRef);
    ...
    if (executionWorkspace.projectId) {
      const projectContext = parseObject(context.project);
      context.project = {
        ...projectContext,
        id: executionWorkspace.projectId,
        ...(readNonEmptyString(projectRecord?.name) ? { name: projectRecord?.name } : {}),
      };
    }
    ...
    try {
      ...
      if (finalizedRun) {
        ...
        if (outcome === "succeeded") {
          await completeBootstrapIssuesOnSuccessfulHeartbeat(agent, finalizedRun);
          await applyStructuredIssueUpdateOnSuccessfulHeartbeat(agent, finalizedRun);
        }
      }
  ...
  async function completeBootstrapIssuesOnSuccessfulHeartbeat(
    agent: typeof agents.$inferSelect,
    run: typeof heartbeatRuns.$inferSelect,
  ) {
    ...
    for (const issue of candidateIssues) {
      if (!isOnboardingBootstrapIssueForAgent(issue, agent)) continue;
      ...
      await logActivity(db, {
        ...
        actorType: "system",
        actorId: "heartbeat_service",
        agentId: agent.id,
        runId: run.id,
        action: "issue.auto_completed",
        entityType: "issue",
        entityId: issue.id,
        details: {
          reason: "onboarding_bootstrap_heartbeat_succeeded",
          title: issue.title,
        },
      });
    }
  ...
  async function applyStructuredIssueUpdateOnSuccessfulHeartbeat(
    agent: typeof agents.$inferSelect,
    run: typeof heartbeatRuns.$inferSelect,
  ) {
    ...
    const update = extractPaperclipIssueUpdate(parseObject(run.resultJson));
    if (!update) return;
    ...
    const commentBody = [update.summary, update.comment]
      .filter((value): value is string => Boolean(value && value.trim().length > 0))
      .join("\n\n");
    if (commentBody) {
      await issuesSvc.addComment(issue.id, commentBody, { agentId: agent.id });
    }

    if (issue.status !== update.status) {
      await issuesSvc.update(issue.id, { status: update.status });
    }
    ...
    await logActivity(db, {
      ...
      agentId: agent.id,
      runId: run.id,
      action: "issue.agent_result_applied",
      entityType: "issue",
      entityId: issue.id,
      details: {
