...
export function issueRoutes(db: Db, storage: StorageService) {
  ...
  function hasMaterialTaskUpdate(
    existing: Record<string, unknown>,
    updateFields: Record<string, unknown>,
  ) {
    const changedScalarField = (field: "title" | "description" | "projectId") =>
      Object.prototype.hasOwnProperty.call(updateFields, field) && existing[field] !== updateFields[field];
    const changedJsonField = (field: "executionWorkspaceSettings" | "assigneeAdapterOverrides") =>
      Object.prototype.hasOwnProperty.call(updateFields, field) &&
      JSON.stringify(existing[field] ?? null) !== JSON.stringify(updateFields[field] ?? null);

    return (
      changedScalarField("title") ||
      changedScalarField("description") ||
      changedScalarField("projectId") ||
      changedJsonField("executionWorkspaceSettings") ||
      changedJsonField("assigneeAdapterOverrides")
    );
  }
  ...
  router.patch("/issues/:id", validate(updateIssueSchema), async (req, res) => {
    ...
    const materialTaskUpdated = hasMaterialTaskUpdate(
    ...
    void (async () => {
      ...
      if (!assigneeChanged && !statusChangedFromBacklog && materialTaskUpdated && issue.assigneeAgentId) {
        wakeups.set(issue.assigneeAgentId, {
          source: "automation",
          triggerDetail: "system",
