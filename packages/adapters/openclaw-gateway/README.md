...
When an OpenClaw `agentId` is provided, legacy Paperclip session keys are automatically scoped to that agent as `agent:<agentId>:<sessionKey>` so gateway validation and per-agent history stay aligned.
...
- optional additions:
  - only gateway-supported top-level `payloadTemplate` fields forwarded (`agentId`, delivery/session fields, `extraSystemPrompt`, `label`, `spawnedBy`, `workspaceDir`, etc.)
  - `agentId` from config if set and not already in template
  - unsupported keys such as top-level `metadata` are ignored to match strict gateway validation

Paperclip context and workspace/runtime hints are embedded into the generated wake text. They are not sent as top-level `paperclip` payload fields because current OpenClaw Gateway `agent` params reject unknown properties.
