...
- payloadTemplate (object, optional): optional OpenClaw agent request fields. Only gateway-supported top-level keys are forwarded ("agentId", delivery/session fields, "extraSystemPrompt", etc.). Unsupported keys are ignored at runtime.
- workspaceRuntime (object, optional): desired runtime service intents for the remote execution environment. Paperclip includes these details in the wake instructions, not as top-level gateway agent params.
...
- when agentId is set, the adapter scopes legacy session keys to that agent as agent:<agentId>:<sessionKey>

Standard wake instructions include:
- Paperclip run, company, agent, issue, and approval identifiers
- resolved workspace hints
- runtime service intent hints when available
