...
} from "@business-factory/adapter-utils";
import { asNumber, asString, buildPaperclipEnv, parseObject } from "@business-factory/adapter-utils/server-utils";
...
const GATEWAY_AGENT_PARAM_TEMPLATE_KEYS = new Set([
  "agentId",
  "to",
  "replyTo",
  "sessionId",
...
const RESERVED_PAYLOAD_TEMPLATE_KEYS = new Set(["message", "text", "idempotencyKey", "sessionKey", "paperclip"]);
...
function parseCanonicalAgentSessionKey(
  sessionKey: string,
): {
  agentId: string;
  rest: string;
  ...
  const agentId = normalizeGatewayAgentId(parts[1] ?? "");
  const rest = parts.slice(2).join(":").trim();
  return agentId && rest ? { agentId, rest } : null;
}

function scopeSessionKeyToAgent(sessionKey: string, agentId: string | null): string {
  const trimmed = sessionKey.trim();
  if (!trimmed || !agentId) return trimmed;
  if (parseCanonicalAgentSessionKey(trimmed)) return trimmed;
  return `agent:${normalizeGatewayAgentId(agentId)}:${trimmed}`;
}
  ...
  const resolved =
    input.strategy === "run"
      ...
      : input.strategy === "issue" && input.issueId
        ...
        : fallback;
  return scopeSessionKeyToAgent(resolved, input.agentId);
}
...
function buildGatewayAgentParams(input: {
  ...
  const supportedTemplate: Record<string, unknown> = {};
  const ignoredKeys: string[] = [];

  for (const [key, value] of Object.entries(input.payloadTemplate)) {
    if (RESERVED_PAYLOAD_TEMPLATE_KEYS.has(key)) continue;
    if (GATEWAY_AGENT_PARAM_TEMPLATE_KEYS.has(key)) {
      supportedTemplate[key] = value;
      continue;
    }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const resolvedAgentId = templateAgentId ?? configuredAgentId;
  ...
  const { agentParams, ignoredKeys: ignoredPayloadTemplateKeys } = buildGatewayAgentParams({
    payloadTemplate,
    message,
    sessionKey,
    runId: ctx.runId,
  ...
  if (ignoredPayloadTemplateKeys.length > 0) {
    await ctx.onLog(
      "stdout",
      `[openclaw-gateway] ignored unsupported payloadTemplate keys: ${ignoredPayloadTemplateKeys.join(", ")}\n`,
    );
  }
  if (transportHint) {
    await ctx.onLog(
