...
import type { Agent, AgentRuntimeState } from "@business-factory/shared";
import { agentsApi } from "../api/agents";
import { useCompany } from "../context/CompanyContext";
import { queryKeys } from "../lib/queryKeys";
import { getAgentAdapterLabel, getAgentDisplayStatus, getAgentRoleLabel, getAgentSubtitle } from "../lib/agent-presentation";
...
export function AgentProperties({ agent, runtimeState }: AgentPropertiesProps) {
  ...
  const displayStatus = getAgentDisplayStatus(agent.status, runtimeState?.lastRunStatus);
      ...
      <div className="space-y-1">
        ...
        <PropertyRow label="Role">
          <span className="text-sm">
            {agent.adapterType === "agent_zero" ? getAgentSubtitle(agent) : getAgentRoleLabel(agent.role)}
          </span>
        </PropertyRow>
        {agent.title && agent.adapterType !== "agent_zero" && (
          <PropertyRow label="Title">
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label="Adapter">
          <span className="text-sm font-mono">{getAgentAdapterLabel(agent.adapterType)}</span>
        </PropertyRow>
      </div>
