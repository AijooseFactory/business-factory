...
import { AGENT_ADAPTER_TYPES } from "@business-factory/shared";
import type {
  Agent,
...
// Adapter metadata moved to capabilities plugins
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
// Canonical type lives in @business-factory/adapter-utils; re-exported here
// so existing imports from this file keep working.
export type { CreateConfigValues } from "@business-factory/adapter-utils";
...
// Options moved to adapter capabilities
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const capabilities = uiAdapter.capabilities || {};
  const isLocal = capabilities.isLocal ?? false;
  ...
  const thinkingEffortKey = capabilities.thinkingEffortKey || "effort";
  const thinkingEffortOptions = capabilities.thinkingEffortOptions || [];
  const currentThinkingEffort = isCreate
    ? val!.thinkingEffort
    : eff("adapterConfig", thinkingEffortKey, String(config[thinkingEffortKey] ?? ""));
  const showThinkingEffort = thinkingEffortOptions.length > 0;
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          <Field label="Adapter type" hint={help.adapterType}>
            <AdapterTypeDropdown
              value={adapterType}
              onChange={(t) => {
                const nextAdapter = getUIAdapter(t);
                const nextCaps = nextAdapter.capabilities || {};
                ...
                if (isCreate) {
                  ...
                  if (nextCaps.defaultModel !== undefined) {
                    nextValues.model = nextCaps.defaultModel;
                  } else if (nextCaps.modelRequired) {
                    nextValues.model = "";
                  }
                  
                  if (nextCaps.defaultAdapterConfig) {
                    Object.assign(nextValues, nextCaps.defaultAdapterConfig);
                  }
                  ...
                  setOverlay((prev) => ({
                    ...
                    adapterType: t,
                    adapterConfig: {
                      model: nextCaps.defaultModel ?? "",
                      effort: "",
                      modelReasoningEffort: "",
                      variant: "",
                      mode: "",
                      ...(nextCaps.defaultAdapterConfig || {}),
                    },
                  }));
                }
              }}
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label="Command" hint={help.localCommand}>
                <DraftInput
                  ...
                  immediate
                  className={inputClass}
                  placeholder={capabilities.defaultCommand || "command"}
                />
              </Field>
              ...
              <ModelDropdown
                ...
                allowDefault={!capabilities.modelRequired}
                required={capabilities.modelRequired ?? false}
                groupByProvider={capabilities.groupByProvider ?? false}
              />
              {fetchedModelsError && (
