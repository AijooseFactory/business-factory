...
type Agent = Pick<SharedAgent, "id" | "name" | "adapterType" | "role" | "title">;
...
export function IssuesList({
  ...
  const agentName = useCallback((id: string | null) => {
    ...
    return agent ? getAgentInlineLabel(agent) : null;
  }, [agents]);
                    ...
                    {agents && agents.length > 0 && (
                      <div className="space-y-1">
                        ...
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {agents.map((agent) => {
                            const agentLabel = getAgentInlineLabel(agent);
                            return (
                            <label key={agent.id} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                              ...
                              />
                              <span className="text-sm">{agentLabel}</span>
                            </label>
                            );
                          })}
                        </div>
                        ...
                        <div className="max-h-48 overflow-y-auto overscroll-contain">
                          ...
                          {(agents ?? [])
                            .filter((agent) => {
                              ...
                              return getAgentInlineLabel(agent).toLowerCase().includes(assigneeSearch.toLowerCase());
                            })
                            .map((agent) => (
                              <button
                                key={agent.id}
                              ...
                              >
                                <Identity name={getAgentInlineLabel(agent)} size="sm" className="min-w-0" />
                              </button>
                            ))}
                        </div>
                      </PopoverContent>
