...
import {
  ONBOARDING_AGENT_PERSONA_BILLING_CODE,
...
const getTaskDescription = (title: string) => {
  if (title === "CEO") {
    return `Create your CEO HEARTBEAT.md and initial organization strategy.

As the CEO, your first task is to establish the ground truth for this company. Download the AGENTS.md, HEARTBEAT.md, SOUL.md, and TOOLS.md templates to your root directory.

Define the company's core values in SOUL.md and outline the initial growth roadmap in HEARTBEAT.md.`;
  }
  return `Review the organization structure and prepare to support the Human CEO. 

Ensure you have a folder agents/chief-of-staff and then download the AGENTS.md, HEARTBEAT.md, SOUL.md, and TOOLS.md templates for your persona.

After you've finished that, prepare to hire additional agents as requested.`;
};
...
export function OnboardingWizard() {
  ...
  // Step 2
  const [agentName, setAgentName] = useState("Einstein");
  const [agentTitle, setAgentTitle] = useState("Chief of Staff");
  const [adapterType, setAdapterType] = useState<AdapterType>("agent_zero");
  ...
  // Step 3
  const [taskTitle, setTaskTitle] = useState("Setup your agent persona");
  const [taskDescription, setTaskDescription] = useState(
    getTaskDescription("Chief of Staff")
  );
  ...
  function reset() {
    ...
    setCompanyName("");
    setCompanyGoal("");
    setAgentName("Einstein");
    setAgentTitle("Chief of Staff");
    setAdapterType("agent_zero");
    setCwd("");
    setModel("");
    setCommand("");
    setArgs("");
    setUrl("http://ajf-einstein:8090/");
    setAdapterEnvResult(null);
    setAdapterEnvError(null);
    setAdapterEnvLoading(false);
    setForceUnsetAnthropicApiKey(false);
    setUnsetAnthropicLoading(false);
    setTaskTitle(`Create your ${agentTitle} HEARTBEAT.md`);
    setTaskDescription(getTaskDescription(agentTitle));
  ...
  async function handleStep2Next() {
    ...
    try {
      ...
      const agent = await agentsApi.create(createdCompanyId, {
        name: agentName.trim(),
        role: agentTitle === "CEO" ? "ceo" : "chief_of_staff",
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                        placeholder="e.g. Einstein, Jarvis"
                        value={agentName}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          className={cn(
                            "flex flex-col items-start gap-1.5 rounded-md border p-4 text-left transition-all",
                            agentTitle === "CEO"
                              ? "border-primary bg-primary/5 ring-1 ring-primary shadow-sm"
                              : "border-border hover:bg-accent/50 text-muted-foreground hover:text-foreground"
                          )}
                          onClick={() => setAgentTitle("CEO")}
                        >
                          <div className="flex items-center justify-between w-full">
                            <span className="font-bold text-sm tracking-tight text-foreground">CEO</span>
                            {agentTitle === "CEO" && <Check className="h-4 w-4 text-primary" />}
                          </div>
                          <span className="text-[11px] leading-snug opacity-90 text-foreground/80">
                            <strong>100% Autonomous AI.</strong> Operates the entire business without human intervention.
                          </span>
                        </button>
                        <button
                          className={cn(
                          ...
                          onClick={() => setAgentTitle("Chief of Staff")}
                        >
                          <div className="flex items-center justify-between w-full">
                            <span className="font-bold text-sm tracking-tight text-foreground">Chief of Staff</span>
                            {agentTitle === "Chief of Staff" && <Check className="h-4 w-4 text-primary" />}
                          </div>
                          <span className="text-[11px] leading-snug opacity-90 text-foreground/80">
                            <strong>Human-in-the-loop.</strong> Collaborates with you and coordinates invited agents.
                          </span>
                        </button>
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        {
                          value: "agent_zero" as const,
                          label: "Agent Zero",
                          icon: Bot,
                          desc: "Connect to your Agent Zero instance"
                        }
                      ].map((opt) => (
                        <button
                          key={opt.value}
                          ...
                          onClick={() => {
                            ...
                            } else if (nextType === "agent_zero" && !url) {
                              setUrl("http://ajf-einstein:8090/");
                            }
                  ...
                  {(adapterType === "http" || adapterType === "openclaw_gateway" || adapterType === "agent_zero") && (
                    <div>
                      <label className="text-xs text-muted-foreground mb-1 block">
                        {adapterType === "openclaw_gateway" ? "Gateway URL" : adapterType === "agent_zero" ? "Agent Zero URL" : "Webhook URL"}
                      </label>
                      <input
                        className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm font-mono outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                        placeholder={adapterType === "openclaw_gateway" ? "ws://127.0.0.1:18789" : adapterType === "agent_zero" ? "http://ajf-einstein:8090/" : "https://..."}
                        value={url}
                  ...
                  <div className="border border-border divide-y divide-border">
                    ...
                    <div className="flex items-center gap-3 px-3 py-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {agentTitle} • {getUIAdapter(adapterType).label}
                        </p>
                      </div>
                      <Check className="h-4 w-4 text-green-500 shrink-0" />
                    </div>
