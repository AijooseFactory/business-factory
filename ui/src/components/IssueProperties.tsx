...
import type { Issue } from "@business-factory/shared";
...
export function IssueProperties({ issue, onUpdate, inline }: IssuePropertiesProps) {
  ...
  const assigneeTrigger = assignee ? (
    <Identity name={getAgentInlineLabel(assignee)} size="sm" />
  ) : assigneeUserLabel ? (
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {sortedAgents
          .filter((a) => {
            ...
            return getAgentInlineLabel(a).toLowerCase().includes(q);
          })
          .map((a) => (
          <button
            key={a.id}
