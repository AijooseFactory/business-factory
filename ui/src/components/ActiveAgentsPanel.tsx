...
import type { Agent, Issue } from "@business-factory/shared";
...
import { selectDashboardAgentRuns } from "../lib/dashboard-runs";
...
export function ActiveAgentsPanel({ companyId }: ActiveAgentsPanelProps) {
  ...
  const runs = useMemo(
    () => selectDashboardAgentRuns(liveRuns ?? [], MIN_DASHBOARD_RUNS),
    [liveRuns],
  );
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-4 xl:grid-cols-4">
          {runs.map((run) => (
            <AgentRunCard
              key={run.id}
              run={run}
              agentLabel={agentById.get(run.agentId) ? getAgentInlineLabel(agentById.get(run.agentId)!) : run.agentName}
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <Identity name={agentLabel} size="sm" />
            </div>
