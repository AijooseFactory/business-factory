...
const BUSINESS_FACTORY_SPRITES = [
  [
    "  ╭────╮ ",
    " ╭╯╭──╮│ ",
    " │ │  ││ ",
...
type BusinessFactorySprite = (typeof BUSINESS_FACTORY_SPRITES)[number];
...
function spriteSize(sprite: BusinessFactorySprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = BUSINESS_FACTORY_SPRITES[(baseCol + baseRow) % BUSINESS_FACTORY_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = BUSINESS_FACTORY_SPRITES[Math.floor(Math.random() * BUSINESS_FACTORY_SPRITES.length)]!;
