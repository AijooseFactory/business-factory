...
import type { ActivityEvent } from "@business-factory/shared";
import type { Agent, IssueAttachment } from "@business-factory/shared";
...
export function IssueDetail() {
  ...
  const mentionOptions = useMemo<MentionOption[]>(() => {
    ...
    for (const agent of activeAgents) {
      options.push({
        id: `agent:${agent.id}`,
        name: getAgentInlineLabel(agent),
        kind: "agent",
      });
    }
  ...
  const commentReassignOptions = useMemo(() => {
    ...
    const activeAgents = [...(agents ?? [])]
      ...
      .sort((a, b) => a.name.localeCompare(b.name));
    for (const agent of activeAgents) {
      options.push({ id: `agent:${agent.id}`, label: getAgentInlineLabel(agent) });
    }
    if (currentUserId) {
              ...
              {childIssues.map((child) => (
                ...
                >
                  ...
                  {child.assigneeAgentId && (() => {
                    ...
                    const name = agent ? getAgentInlineLabel(agent) : null;
                    return name
