...
import type { ApprovalComment } from "@business-factory/shared";
...
export function ApprovalDetail() {
  ...
  const agentNameById = useMemo(() => {
    ...
    for (const agent of agents ?? []) map.set(agent.id, getAgentInlineLabel(agent));
    return map;
  }, [agents]);
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: (resolvedApproval) => {
      setError(null);
      refresh();
      navigate(`/approvals/${approvalId}?resolved=approved`, {
        replace: true,
        state: {
          claimSecret: resolvedApproval.claimSecret ?? null,
          claimApiKeyPath: resolvedApproval.claimApiKeyPath ?? null,
        },
      });
    },
  ...
  const adapterType = typeof payload.adapterType === "string" ? payload.adapterType : null;
  const isActionable = approval.status === "pending" || approval.status === "revision_requested";
  const TypeIcon = typeIcon[approval.type] ?? defaultTypeIcon;
  const showApprovedBanner = searchParams.get("resolved") === "approved" && approval.status === "approved";
  const claimState =
    typeof location.state === "object" && location.state !== null && !Array.isArray(location.state)
      ? (location.state as { claimSecret?: string; claimApiKeyPath?: string })
      : null;
  const claimSecret = typeof claimState?.claimSecret === "string" ? claimState.claimSecret : null;
  const claimApiKeyPath =
    typeof claimState?.claimApiKeyPath === "string"
      ? claimState.claimApiKeyPath
      : typeof approval.claimApiKeyPath === "string"
        ? approval.claimApiKeyPath
        : null;
  const claimExpiryLabel = approval.claimSecretExpiresAt
    ? new Date(approval.claimSecretExpiresAt).toLocaleString()
    : null;
  const showOpenClawBootstrapClaim =
    approval.type === "hire_agent" &&
    approval.status === "approved" &&
    adapterType === "openclaw_gateway" &&
    (claimApiKeyPath !== null || claimSecret !== null);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {showOpenClawBootstrapClaim && (
        <div className="border border-blue-300 dark:border-blue-700/40 bg-blue-50 dark:bg-blue-950/20 rounded-lg px-4 py-3 space-y-2">
          <div>
          ...
          {claimSecret && (
            <div className="space-y-1">
              <p className="text-[11px] uppercase tracking-wide text-blue-800/80 dark:text-blue-200/80">Claim secret</p>
              <p className="font-mono text-xs break-all text-blue-950 dark:text-blue-50">{claimSecret}</p>
            </div>
          )}
          {claimApiKeyPath && (
            <div className="space-y-1">
              <p className="text-[11px] uppercase tracking-wide text-blue-800/80 dark:text-blue-200/80">Claim path</p>
              <p className="font-mono text-xs break-all text-blue-950 dark:text-blue-50">POST {claimApiKeyPath}</p>
            </div>
          )}
          {claimExpiryLabel && (
            <p className="text-xs text-blue-800 dark:text-blue-200/90">Expires: {claimExpiryLabel}</p>
          )}
          {!claimSecret && (
            <p className="text-xs text-blue-800 dark:text-blue-200/90">
              The one-time secret is only shown in the approval response. If you no longer have it, create an agent API key manually from the hired agent page.
            </p>
          )}
        </div>
      )}
