...
import type { Agent, Issue, LiveEvent } from "@business-factory/shared";
import { authApi } from "../api/auth";
import { useCompany } from "./CompanyContext";
import type { ToastInput } from "./ToastContext";
import { useToast } from "./ToastContext";
import { getAgentInlineLabel, getAgentSubtitle } from "../lib/agent-presentation";
...
function buildAgentStatusToast(
  ...
  const agents = queryClient.getQueryData<Agent[]>(queryKeys.agents.list(companyId));
  const agent = agents?.find((a) => a.id === agentId);
  const body = agent && agent.adapterType !== "agent_zero" ? getAgentSubtitle(agent) : undefined;
