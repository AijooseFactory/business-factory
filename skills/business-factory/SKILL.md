...
description: >
  Interact with the Business Factory control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, or call any
  Business Factory API endpoint. Do NOT use for the actual domain work itself (writing
  code, research, etc.) — only for Business Factory coordination.
---

# Business Factory Skill

You run in **heartbeats** — short execution windows triggered by Business Factory. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.
...
Manual local CLI mode (outside heartbeat runs): use `business-factoryai agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install Business Factory skills for Claude/Codex and print/export the required `PAPERCLIP_*` environment variables for that agent identity.

**Run audit trail:** You MUST include `-H 'X-Business Factory-Run-Id: $PAPERCLIP_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.
...
Headers: Authorization: Bearer $PAPERCLIP_API_KEY, X-Business Factory-Run-Id: $PAPERCLIP_RUN_ID
...
Headers: X-Business Factory-Run-Id: $PAPERCLIP_RUN_ID
...
- **Hiring**: use `business-factory-create-agent` skill for new agent creation workflows.
- **Commit Co-author**: if you make a git commit you MUST add `Co-Authored-By: Business Factory <noreply@business-factory.ing>` to the end of each commit message
...
Use this when validating Business Factory itself (assignment flow, checkouts, run visibility, and status transitions).

1. Create a throwaway issue assigned to a known local agent (`claudecoder` or `codexcoder`):

```bash
pnpm business-factoryai issue create \
...
```bash
pnpm business-factoryai heartbeat run --agent-id "$PAPERCLIP_AGENT_ID"
```
...
```bash
pnpm business-factoryai issue update <issue-id> --assignee-agent-id <other-agent-id> --status todo
```

5. Cleanup: mark temporary issues done/cancelled with a clear note.

If you use direct `curl` during these tests, include `X-Business Factory-Run-Id` on all mutating issue requests whenever running inside a heartbeat.

## Full Reference

For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/business-factory/references/api-reference.md`
