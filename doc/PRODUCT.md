# Business Factory — Product Definition

## What It Is

Business Factory is the control plane for autonomous AI companies. One instance of Business Factory can run multiple companies. A **company** is a first-order object.
...
- **Adapter type + config** — how this agent runs and what defines its identity/behavior. This is adapter-specific (e.g., an OpenClaw agent might use SOUL.md and HEARTBEAT.md files; a Claude Code agent might use CLAUDE.md; a bare script might use CLI args). Business Factory doesn't prescribe the format — the adapter does.
...
1. **Run a command** — Business Factory kicks off a process (shell command, Python script, etc.) and tracks it. The heartbeat is "execute this and monitor it."
2. **Fire and forget a request** — Business Factory sends a webhook/API call to an externally running agent. The heartbeat is "notify this agent to wake up." (OpenClaw hooks work this way.)
...
5. **Control plane, not execution plane.** Business Factory orchestrates. Agents run wherever they run and phone home.

## User Flow (Dream Scenario)

1. Open Business Factory, create a new company
...
There are two runtime modes Business Factory must support:
...
Business Factory’s core identity is a **control plane for autonomous AI companies**, centered on **companies, org charts, goals, issues/comments, heartbeats, budgets, approvals, and board governance**. The public docs are also explicit about the current boundaries: **tasks/comments are the built-in communication model**, Business Factory is **not a chatbot**, and it is **not a code review tool**. The roadmap already points toward **easier onboarding, cloud agents, easier agent configuration, plugins, better docs, and ClipMart/ClipHub-style reusable companies/templates**.

## What Business Factory should do vs. not do
...
- Do not build a complete Jira/GitHub replacement. The repo/docs already position Business Factory as organization orchestration, not focused on pull-request review.
