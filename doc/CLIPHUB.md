...
ClipHub is the public registry where people share, discover, and download Business Factory company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Business Factory what a package registry is to a programming language. Business Factory already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Business Factory — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Business Factory instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Business Factory spec. It contains:
...
**Install (fresh start):**
```
business-factory install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Business Factory instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Business Factory, export your company as a template, then publish:

```
business-factory export --template my-company
business-factory publish cliphub my-company
```
...
ClipHub is a **separate service** from Business Factory itself. Business Factory is self-hosted; ClipHub is a hosted registry that Business Factory instances talk to.
...
| **Business Factory CLI** | `business-factoryai install`, `business-factoryai publish`, `business-factoryai cliphub sync` — built into Business Factory |
| **Business Factory UI** | "Browse ClipHub" panel in the Business Factory web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Business Factory) |
| Backend | TypeScript + Hono (consistent with Business Factory) |
| Database | PostgreSQL |
...
4. Run `business-factoryai install cliphub:acme/lean-saas-shop`
5. Business Factory creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Business Factory until it works well
2. Export: `business-factoryai export --template my-agency`
3. Publish: `business-factoryai publish cliphub my-agency`
...
3. Install just that agent: `business-factoryai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Business Factory

ClipHub is **not required** to use Business Factory. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Business Factory what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `business-factoryai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Business Factory web UI
- [ ] `business-factoryai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Business Factory's own philosophy)
