...
Audience: Operators setting up and running agents in Business Factory

## 1. What this system does

Agents in Business Factory do not run continuously.  
...
For `claude_local` and `codex_local`, Business Factory assumes the CLI is already installed and authenticated on the host machine.
...
Business Factory stores resumable session state per `(agent, taskKey, adapterType)`.
...
Business Factory pushes runtime/activity updates to the browser in real time.
