...
This spec defines how Business Factory actually runs agents while staying runtime-agnostic.
...
1. Business Factory is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Business Factory manages lifecycle, and on exit Business Factory parses JSON output and updates state.
...
```json
{
  "runLogStore": {
    ...
    "bucket": "business-factory-run-logs",
    "prefix": "runs/",
    "compress": true,
...
- `run.startedAt`
- `heartbeat.reason`
- `business-factory.skill` (shared Business Factory skill text block)
