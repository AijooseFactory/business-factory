...
Keep Business Factory low-friction while making the mode model simpler and safer:
...
```sh
pnpm business-factoryai onboard
```
...
```sh
pnpm business-factoryai configure --section server
```
...
```sh
pnpm business-factoryai doctor
```
...
1. `pnpm business-factoryai onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm business-factoryai doctor` works flagless with mode-aware checks.
