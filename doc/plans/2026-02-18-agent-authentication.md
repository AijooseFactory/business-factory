...
Agents need API keys to authenticate with Business Factory. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Business Factory
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Business Factory generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Business Factory CLI
...
```
business-factory auth login
# Opens browser -> user confirms -> token stored at ~/.business-factory/credentials
```
...
managed by the Business Factory server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Business Factory.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Business Factory SKILL.md (or a link to it)
   - What information Business Factory needs from the agent (e.g. webhook URL, adapter
...
5. Business Factory stores the pending registration.
...
7. On approval, Business Factory provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Business Factory issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.business-factory.ing/skills/business-factory/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Business Factory should send you heartbeats",
...
1. Generate a Business Factory invite link for the company.
...
4. A Business Factory company member approves the new agent.
5. Business Factory begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `business-factoryai auth login` for developer-managed remote agents.                                      |
