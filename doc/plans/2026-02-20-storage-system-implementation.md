...
Add a single storage subsystem for Business Factory that supports:
...
- API serves attachment bytes through authenticated Business Factory endpoints.
...
- Default local path is under instance root: `~/.business-factory/instances/<instanceId>/data/storage`.
...
- `business-factoryai onboard` writes a valid `storage` config block by default.
- `business-factoryai configure --section storage` can switch between local and s3 modes.
...
- `business-factoryai doctor` reports actionable storage status.
