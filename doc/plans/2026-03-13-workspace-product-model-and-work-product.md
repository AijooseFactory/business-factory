...
Business Factory needs to support two very different but equally valid ways of working:
...
Today, Business Factory already has the beginnings of this model:
...
- making Business Factory feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Business Factory users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Business Factory deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Business Factory into a full code review product
...
This object must be recorded explicitly so that Business Factory can:
...
Business Factory should treat PRs as a type of work product linked back to:
...
Business Factory already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Business Factory must be a first-class deployment mode

Business Factory cannot assume that it is running on the same machine as the code.

In cloud deployments, Business Factory may:
...
- `Runtime service`: a process or service Business Factory owns or tracks for a workspace
...
It also lets cloud-hosted Business Factory deployments point at codebases and remotes without pretending the Business Factory host has direct filesystem access.
...
This lets Business Factory support:
...
Without an explicit `execution workspace` record, Business Factory has nowhere stable to attach:
...
Business Factory needs a single place to show:
...
- Business Factory-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Business Factory owning the execution host
...
## 1. Business Factory host and execution host must be decoupled

Business Factory may run:
...
without the Business Factory host itself running local git or local preview processes.
...
This is how Business Factory supports:
...
That keeps Business Factory focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Business Factory deployments remain first-class

That is a better fit for Business Factory than either extreme:
