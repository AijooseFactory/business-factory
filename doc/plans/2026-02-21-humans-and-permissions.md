...
- local mode that still feels instant (`npx business-factoryai run` and go)
...
2. CLI command `pnpm business-factoryai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm business-factoryai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm business-factoryai onboard`).
...
- expose key instance settings in API + CLI (`business-factoryai configure` / `business-factoryai onboard`)
...
7. `pnpm business-factoryai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
