...
This document defines the V1 design for a Business Factory adapter that integrates with
...
4. synthesized stdout events for Business Factory UI/CLI
...
3. **Agent callback auth to Business Factory** uses a bootstrap exchange flow (no long-lived Business Factory key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Business Factory endpoints, not full SKILL.md prompt injection.
...
- `business-factoryPublicUrl` (optional override; else `PAPERCLIP_PUBLIC_URL` env)
...
## Business Factory Callback + Auth Flow (V1)
...
1. `adapterConfig.business-factoryPublicUrl`
...
Goal: avoid putting long-lived Business Factory credentials in prompt text.

Flow:

1. Before launch/follow-up, Business Factory mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
   - short TTL (for example 10 minutes)
2. Adapter includes only:
   - `business-factoryPublicUrl`
   - exchange endpoint path
   - bootstrap token
3. Cursor agent calls:
   - `POST /api/agent-auth/exchange`
4. Business Factory validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Business Factory API calls.
...
1. Prompt includes a compact instruction to fetch skills from Business Factory.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/business-factory`
   - `GET /api/skills/business-factory-create-agent` when needed
...
- resolve `business-factoryPublicUrl`
...
- include webhook object when enabled:
  - `url: <business-factoryPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Business Factory run context.
...
5. `business-factoryPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- business-factory public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from Business Factory skill endpoints |
| Business Factory auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Business Factory context.
3. Add richer PR metadata surfacing in Business Factory UI.
