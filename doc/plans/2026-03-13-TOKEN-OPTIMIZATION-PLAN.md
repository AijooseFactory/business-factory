...
Date: 2026-03-13  
Related discussion: https://github.com/business-factoryai/business-factory/discussions/449
...
- local runtime data from the default Business Factory instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `business-factory` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `business-factory` skill currently tells agents to do this on essentially every heartbeat:
...
- Business Factory discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Business Factory skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Business Factory-side skill changes land.
...
- `skills/business-factory/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/business-factory-create-agent/SKILL.md`: 4,718 bytes
...
- Store both:
  ...
  - Business Factory-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Business Factory to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `business-factory` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat Business Factory-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `business-factory` skill rewrite
...
6. Rewrite `skills/business-factory/SKILL.md` around delta-fetch behavior.
