# Business Factory Module System

## Overview

Business Factory's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Business Factory's API, UI, and data model | Yes |
...
modules/
  observability/
    business-factory.module.json     # manifest (required)
    src/
...
### Manifest (`business-factory.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@business-factoryai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@business-factoryai/core";
...
import type { Db } from "@business-factoryai/db";
...
```
1. Scan modules/ directory for business-factory.module.json manifests
...
```jsonc
// business-factory.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@business-factoryai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@business-factoryai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:business-factory/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.business-factory.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm business-factoryai store list                    # browse available modules and templates
pnpm business-factoryai store install <module-id>     # install a module
pnpm business-factoryai store import <template-id>    # import a company template
pnpm business-factoryai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Business Factory instance | `server:started` |
...
Add to `@business-factoryai/server`:
...
4. **Module config** — `business-factory.config.json` with per-module config, env var interpolation
...
10. **`@business-factoryai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
