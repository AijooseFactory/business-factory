...
`PAP-447` asks how Business Factory should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Business Factory instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Business Factory:

- some operators use Business Factory to manage Business Factory and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Business Factory
- Claude and Codex expose different built-in affordances, so Business Factory should not overfit to one tool

## Core Product Decision

Business Factory should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Business Factory
...
- Business Factory should not assume that every adapter can see or use a host filesystem path directly
...
If Business Factory makes worktrees agent-first, it will blur:
...
- Business Factory core resolves and records execution workspace state
...
That gives Business Factory room to support:
...
- Business Factory handles workspace runtime services under the hood using project/workspace policy
...
Once Business Factory is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Business Factory should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Business Factory cannot infer
...
- when issue moves to done, should Business Factory auto-commit?
- should it auto-push?
...
Business Factory already has the right foundation for a project-first model.
...
For server-spawned local adapters, Business Factory already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Business Factory should mirror directly

Implication:

- for `codex_local`, Business Factory should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Business Factory should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Business Factory should separate:
...
Business Factory should be able to express intentions such as:
...
These are not specific to Business Factory. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".business-factory/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Business Factory-specific `devServer` flag.
...
Business Factory should distinguish between:
...
Business Factory should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
### Business Factory Core

Business Factory core should:
...
Business Factory core should not:
...
In addition to the local git helper, Business Factory should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Business Factory
...
For runtime services specifically, Business Factory should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Business Factory-only dev-server flags
...
3. Inject resolved execution workspace details into `context.business-factoryWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Business Factory instance can be reused through env/config injection when appropriate
...
- Business Factory can decide whether to reuse or start a fresh service deterministically
...
3. Test against Business Factory-on-Business Factory workflows first.
...
7. Workspace runtime services are modeled generically, not as Business Factory-specific dev-server toggles.
