# Business Factory Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Business Factory's plugin and extension architecture.
...
Business Factory plugin design is based on the following assumptions:

1. Business Factory is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Business Factory business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Business Factory core.
...
2. Let plugins add major capabilities without editing Business Factory core.
...
4. Depend on project-local plugin folders such as `.business-factory/plugins`.
...
The single Business Factory deployment an operator installs and controls.

### 5.2 Company

A first-class Business Factory business object inside the instance.
...
A trusted in-process extension loaded directly by Business Factory core.
...
An installable instance-wide extension package loaded through the Business Factory plugin runtime.
...
Business Factory has two extension classes.
...
Business Factory already has a concrete workspace model:
...
Plugins live under the Business Factory instance directory.

Suggested layout:

- `~/.business-factory/instances/default/plugins/package.json`
- `~/.business-factory/instances/default/plugins/node_modules/`
- `~/.business-factory/instances/default/plugins/.cache/`
- `~/.business-factory/instances/default/data/plugins/<plugin-id>/`

The package install directory and the plugin data directory are separate.

## 8.2 Operator Commands

Business Factory should add CLI commands:

- `pnpm business-factoryai plugin list`
- `pnpm business-factoryai plugin install <package[@version]>`
- `pnpm business-factoryai plugin uninstall <plugin-id>`
- `pnpm business-factoryai plugin upgrade <plugin-id> [version]`
- `pnpm business-factoryai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@business-factory/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```json
{
  "name": "@business-factory/plugin-linear",
  "version": "0.1.0",
  "business-factoryPlugin": {
...
```ts
export interface Business FactoryPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumBusiness FactoryVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Business Factory agents can use during runs.
...
- Business Factory server starts one worker process per installed plugin
...
Receives one typed Business Factory domain event.
...
export function definePlugin(definition: PluginDefinition): Business FactoryPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: Business FactoryPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@business-factoryai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@business-factoryai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@business-factoryai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@business-factoryai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@business-factoryai/plugin-sdk/ui`
...
The `@business-factoryai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Business Factory secret provider system rather than a plain text input
...
1. Core Business Factory data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Business Factory objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Business Factory product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Business Factory later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Business Factory secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm business-factoryai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Business Factory server. This is a normative requirement, not optional.
...
### 27.1 `@business-factoryai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@business-factoryai/plugin-test-harness";
...
For developing a plugin against a running Business Factory instance:

- The operator installs the plugin from a local path: `pnpm business-factoryai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-business-factory-plugin`) that scaffolds:

- `package.json` with correct `business-factoryPlugin` keys
...
- `@business-factory/plugin-workspace-files`
- `@business-factory/plugin-terminal`
- `@business-factory/plugin-git`
- `@business-factory/plugin-linear`
- `@business-factory/plugin-github-issues`
- `@business-factory/plugin-grafana`
- `@business-factory/plugin-runtime-processes`
- `@business-factory/plugin-stripe`
...
- `@business-factoryai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@business-factoryai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@business-factoryai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@business-factoryai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@business-factoryai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@business-factoryai/plugin-sdk/ui`
...
- `@business-factoryai/plugin-test-harness`
- `create-business-factory-plugin` starter template
...
Business Factory should not implement a generic in-process hook bag modeled directly after local coding tools.

Business Factory should implement:
...
That is the complete target design for the Business Factory plugin system.
