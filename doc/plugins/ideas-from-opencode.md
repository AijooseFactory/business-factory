...
Business Factory V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Business Factory-shaped design.

Assumption for this document: Business Factory is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Business Factory objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Business Factory.

The main conclusion is:

- Business Factory should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Business Factory should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Business Factory should use multiple extension classes instead of one generic plugin bag:
...
If Business Factory does this well, the examples you listed become straightforward:
...
Relevant Business Factory files reviewed for current extension seams:
...
It is too dangerous for Business Factory core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Business Factory — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Business Factory should absolutely do this.
...
Business Factory should copy this discipline.
...
Business Factory should adopt this style for plugin actions, automations, and UI schemas.
...
Business Factory can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Business Factory too.

## What Business Factory Should Not Copy Directly
...
Business Factory is a control plane for an operator-managed instance with company objects.
...
Business Factory is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Business Factory should not auto-load arbitrary code from a workspace repo like `.business-factory/plugins` or project directories.
...
For Business Factory, equivalent hooks into:
...
Business Factory should not allow plugins to silently replace:
...
For Business Factory it would be risky because it combines:
...
Business Factory should require an explicit operator install step.

## Why Business Factory Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Business Factory |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Business Factory instance |
...
That means Business Factory should borrow the good ideas from `opencode` but use a stricter architecture.

## Business Factory Already Has Useful Pre-Plugin Seams

Business Factory has several extension-like seams already:
...
This is good news.
Business Factory does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Business Factory Plugin Model
...
Business Factory already has this pattern implicitly:
...
- `plugin` means integration code that talks to Business Factory through a typed plugin protocol and capability model
...
- invoke explicit Business Factory actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@business-factoryai/plugin-sdk/ui`) with design tokens and common components.
...
Business Factory should treat plugin installation as a global instance-level action.

Examples:

- install `@business-factory/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Business Factory objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Business Factory already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Business Factory too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@business-factory/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@business-factoryai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Business Factory instance.

A `create-business-factory-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Business Factory achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Business Factory server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Business Factory's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@business-factoryai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@business-factoryai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Business Factory

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@business-factoryai/plugin-sdk";

export default definePlugin({
  id: "@business-factory/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Business Factory entities
    });
    ...
    ctx.events.on("plugin.@business-factory/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@business-factoryai/plugin-sdk/ui";
...
Any Business Factory plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Business Factory objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@business-factory/plugin-linear`)
...
- reuse core tables when the data is clearly part of Business Factory itself
...
If a concept is becoming part of Business Factory's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Business Factory itself
...
### 3. Scope plugin data to Business Factory objects before adding custom schemas

A lot of plugin data naturally hangs off existing Business Factory objects:
...
If Business Factory eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@business-factory/plugin-workspace-files`
...
Package idea: `@business-factory/plugin-terminal`
...
Package idea: `@business-factory/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Business Factory.
- Board reviews the diff after a run without leaving Business Factory.
...
- optional `events.emit` (e.g. `plugin.@business-factory/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@business-factory/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@business-factory/plugin-linear`

This plugin syncs Business Factory work with Linear. It is useful for:

- importing backlog from Linear
- linking Business Factory issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Business Factory and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Business Factory.
- Business Factory issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@business-factory/plugin-github-issues`

This plugin syncs Business Factory issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Business Factory issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Business Factory task governance
...
- Plugin settings:
  ...
  - whether new Business Factory issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Business Factory issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Business Factory.
- GitHub webhooks update status/comment state in Business Factory.
- A PR is linked back to the Business Factory issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@business-factory/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@business-factory/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Business Factory. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Business Factory issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Business Factory plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Business Factory dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Business Factory issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Business Factory core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@business-factory/plugin-runtime-processes`
...
Package idea: `@business-factory/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Business Factory. It is useful for:
...
- Failed payment spikes or churn events can generate Business Factory issues for follow-up.
...
- Stripe data should stay additive to Business Factory core
...
- plugin UI bundle loading, host bridge, `@business-factoryai/plugin-sdk/ui`
...
- `@business-factoryai/plugin-test-harness` and `create-business-factory-plugin` starter template
...
Business Factory should not implement "an OpenCode-style generic in-process hook system."
Business Factory should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Business Factory
...
8. Ship `@business-factoryai/plugin-test-harness` and `create-business-factory-plugin` starter template.
